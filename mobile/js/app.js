(function () {
  ...
  function ensureProfileLink(doc) {
      ...
      modalContents.forEach(content => {
          const ensureLink = (href, icon, label) => {
              let link = content.querySelector(`a[href="${href}"]`);
              if (!link) {
                  link = doc.createElement('a');
                  link.href = href;
                  link.setAttribute('data-nav', 'soft');
          ...
          const profileLink = ensureLink('/profile', 'fa-solid fa-user', 'Profilim');
          const settingsLink = ensureLink('/settings', 'fa-solid fa-cog', 'Ayarlar');
          const logoutLink = ensureLink('/logout', 'fa-solid fa-right-from-bracket', 'Çıkış Yap');

          const anchor = content.querySelector('.lang-switcher');
          if (anchor && anchor.nextSibling !== profileLink) {
              content.insertBefore(profileLink, anchor.nextSibling);
          } else if (!anchor) {
              content.prepend(profileLink);
          }
          if (profileLink.nextSibling !== settingsLink) {
              content.insertBefore(settingsLink, profileLink.nextSibling);
          }
          if (settingsLink.nextSibling !== logoutLink) {
              content.insertBefore(logoutLink, settingsLink.nextSibling);
          }
      });
  }
  ...
  function injectShellStyles() {
      let style = document.getElementById('universal-shell-styles');
      if (!style) {
      ...
      style.textContent = `
          ...
          body.shell-active .wrap {
              margin-top: 0 !important;
          }
          body.shell-active main,
          body.shell-active.has-topbar main,
          body.shell-active.has-topbar .main-content,
          body.shell-active.has-topbar .page-content,
          body.shell-active.has-topbar .server-main {
          ...
          .server-main::-webkit-scrollbar { width: 0; height: 0; display: none; }
          .server-main::-webkit-scrollbar-thumb { display: none; }
          /* Normalize page-specific shells rendered inside global shell */
          .server-main .cx-shell,
          .server-main .app-shell,
          .server-main .voice-shell,
          .server-main .settings-shell,
          .server-main .page-shell {
              height: auto !important;
              min-height: 0 !important;
              overflow: visible !important;
          }
          .server-main .cx-content,
          .server-main .main-content,
          .server-main .page-content {
              min-height: 0 !important;
              overflow: visible !important;
          }
          .server-rail {
          ...
          .server-rail .rail-btn-wrapper::before {
          ...
          .server-rail .rail-btn,
          .server-rail .rail-avatar {
          ...
          .server-rail .rail-btn-wrapper:hover .rail-btn,
          .server-rail .rail-btn:hover {
              background: rgba(var(--bg-rgb), 0.28) !important;
              border-color: rgba(var(--line-rgb), 0.65) !important;
              color: var(--text) !important;
              transform: none !important;
              box-shadow: none !important;
          }
          .server-rail .rail-btn:hover .rail-label {
              color: var(--text) !important;
          }
          .rail-btn.active {
              background: linear-gradient(165deg, rgba(var(--primary-rgb), 0.34), rgba(var(--primary-rgb), 0.2));
              border-color: rgba(var(--primary-rgb), 0.55);
              color: #ffffff;
          }
          .rail-btn.active .rail-label { color: #ffffff; }
          .rail-btn.active .rail-avatar { border-color: rgba(255, 255, 255, 0.7); }
          .rail-btn:active { transform: scale(0.98); }
          .rail-divider,
          .server-rail [style*="flex:1"] { display: none !important; }
  ...
  function railMarkup(avatarSrc) {
      const safeAvatar = avatarSrc || '/mobile/avatar.svg';
      return `
          <div class="rail-btn-wrapper">
              <a class="rail-btn" href="/" data-nav="soft" title="Anasayfa">
                  <i class="fa-solid fa-house"></i>
                  <span class="rail-label">Anasayfa</span>
              </a>
          </div>
          <div class="rail-btn-wrapper">
              <a class="rail-btn" href="/dm" data-nav="soft" title="DM">
                  <i class="fa-solid fa-paper-plane"></i>
                  <span class="rail-label">DM</span>
              </a>
          </div>
          <div class="rail-btn-wrapper">
              <a class="rail-btn" href="/casino" data-nav="soft" title="Casino">
                  <i class="fa-solid fa-dice"></i>
                  <span class="rail-label">Casino</span>
              </a>
          </div>
          <div class="rail-btn-wrapper">
          ...
          <div class="rail-btn-wrapper">
              <a class="rail-btn" href="/servers" data-nav="soft" title="Sunucular">
                  <i class="fa-solid fa-server"></i>
                  <span class="rail-label">Sunucular</span>
              </a>
          </div>
          <div class="rail-btn-wrapper">
              <button class="profile-btn rail-btn rail-profile" id="profileBtn" type="button" title="Profil">
                  <img src="${safeAvatar}" class="rail-avatar" alt="avatar">
                  <span class="rail-label">Profil</span>
              </button>
          </div>
      `;
  }

  function detectAvatarSource(root = document) {
      const selectors = [
          '.topbar .avatar',
          '.server-rail .rail-avatar',
          '.profile-btn .avatar',
  ...
  function ensureServerShell() {
      ...
      const avatarSrc = detectAvatarSource(document);
      if (oldTopbar) oldTopbar.remove();

      let shell = document.querySelector('.server-shell');
      if (!shell) {
          const oldMain = document.querySelector('main');
          const oldPageShell = document.querySelector('.cx-shell, .app-shell, .voice-shell, .settings-shell, .page-shell');
          ...
          let rail = shell.querySelector('.server-rail');
          if (!rail) {
          ...
          if (!shell.querySelector('.server-main')) {
              const existingMain = shell.querySelector('main') || document.querySelector('main');
              const serverMain = document.createElement('main');
              serverMain.className = 'server-main';
              if (existingMain && existingMain !== serverMain) {
                  existingMain.style.paddingTop = '0';
                  serverMain.appendChild(existingMain);
              } else {
                  const looseContent = Array.from(document.body.children).filter((node) => {
                      if (!node || !node.tagName) return false;
  ...
  window.updateRailActiveState = function(pathOverride) {
      ...
      document.querySelectorAll('.server-rail a.rail-btn').forEach(btn => {
      ...
      const profileBtn = document.querySelector('.server-rail .rail-profile');
      if (profileBtn) {
          const active = profileRoutes.some((base) => newPath === base || newPath.startsWith(`${base}/`));
  ...
  async function updateDynamicRail() {
      try {
          ...
          const currentDataStr = String(data.avatar || '');
          ...
          if (!rail) return;
          rail.innerHTML = railMarkup(data.avatar || detectAvatarSource(document));
  ...
  window.openServerDrawer = function() {
      const root = document.querySelector('.server-main');
      if (!root || !root.querySelector('.server-sidebar')) return;
  ...
  window.toggleServerDrawer = function() {
      const root = document.querySelector('.server-main');
      if (!root || !root.querySelector('.server-sidebar')) return;
  ...
  function initServerDrawer() {
      const root = document.querySelector('.server-main');
      if (!root || !root.querySelector('.server-sidebar')) return;
      root.querySelectorAll('.chan-link[data-nav="soft"]').forEach((a) => {
          a.addEventListener('click', () => window.closeServerDrawer());
      });
  }
  ...
  function swapFromHtml(html, url, replaceOnly, isSubmit) {
    ...
    // Keep shell/nav styles last so page-level styles cannot override mobile nav layout.
    injectShellStyles();
    ...
    if (!nextShell && currentShell) {
        document.body.classList.add('shell-active');
            const incomingRoot = doc.querySelector('.cx-shell, .app-shell, .voice-shell, .settings-shell, .page-shell') || doc.querySelector('main');
        if (incomingRoot) {
            const currentMain = currentShell.querySelector('.server-main');
            if (currentMain) {
                currentMain.innerHTML = '';
                if (incomingRoot.tagName && incomingRoot.tagName.toLowerCase() === 'main') incomingRoot.style.paddingTop = '0';
                currentMain.appendChild(incomingRoot);
            }
        }
