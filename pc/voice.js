(function () {
  ...
  async function postSignal(target, kind, payload) {
    ...
    await fetch(`/voice/signal/${serverId}/${channelId}`, { method: 'POST', body: fd, headers: { 'X-Requested-With': 'fetch' } });
  }
  ...
  async function refreshUsers() {
    const res = await fetch(`/voice/ping/${serverId}/${channelId}`, { method: 'POST', headers: { 'X-Requested-With': 'fetch' } });
  ...
  async function joinVoice() {
    ...
    setInterval(refreshUsers, 12000);
    joinBtn.disabled = true;
  }

  joinBtn?.addEventListener('click', () => joinVoice().catch(() => {}));
